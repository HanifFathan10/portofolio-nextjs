...
const EmailSection = ({ id }: { id: any }) => {
  ...
  const sendEmail = async (e: any) => {
    ...
    try {
      await emailjs.sendForm("service_uslipyo", "template_2atmtkp", form.current, "iT91ZsthnS02_yFOG").then((result) => {
        if (result) {
  ...
  const sendMessage: any = (result: any) => {
    const Email = form.current.email.value;
    const Subject = form.current.subject.value;
    const Message = form.current.message.value;
    if (result && Email && Subject && Message) {
        ...
        <div>
          <form ref={form} onSubmit={sendEmail} className="flex flex-col gap-4 z-[100]">
            ...
            <div className="mb-4">
              ...
              <textarea name="message" id="message" className="bg-[#18191E] border border-[#9CA2A9] text-gray-100 text-sm rounded-lg block w-full p-2" placeholder="Let's talk about..." cols={5} rows={5} required />
            </div>
            <button type="submit" onClick={sendMessage} className="bg-sky-800 hover:bg-cyan-300 text-white hover:text-[#212121] font-medium py-2.5 px-5 transition-all rounded-lg w-full focus:ring-2 focus:ring-slate-500">
              Send Message
            </button>
            <div className="">
              {isMessageSent && (
