    ...
    <section ref={ref} className="flex flex-col justify-center items-center" id={id}>
      ...
      <ul className="flex max-w-xs md:max-w-xl mt-10 justify-center items-center">
        {ProjectData.map((project: Iproject, index) => {
          return (
            <motion.li variants={cartVariants} initial="initial" animate={isInView ? "animate" : "initial"} transition={{ duration: 0.3, delay: (index = 0.4) }} key={index}>
              <ProjectCart key={project.id} title={project.title} description={project.description} imgUrl={project.image} gitUrl={project.gitUrl} previewUrl={project.previewUrl} />
            </motion.li>
          );
        })}
      </ul>
